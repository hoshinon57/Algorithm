...
int main(void)
{
	...
	※ただし、マルチテストケースの場合はサンプル以外全てWA,ということもありえる
	...
	*/
	/*
	AtCoderにてTLEが出た場合のチェックポイント
	特に、テストケースのごく一部(2,3つ)のみTLEするといったケース。
	(1)どういった入力のときに時間がかかるのか試行錯誤する。
	(2)setはpriority_queueに比べて全般遅い。
	   以下のように置き換えることでACするケースも多々ある。
	   ・最大/最小など集合内の特定要素にしかアクセスしない場合、setをpqへ
	   ・末尾に対して追加削除を繰り返す場合、vectorへ
	   ex. ABC390-D, ABC409-F
	   ※特に登録(set::insert, pq::push)はsetが突出して遅い。(一概に比較はできないが、10倍ぐらい遅い？)
	     ただ逆に、削除(set::erase, pq::pop)はpqの方が遅い、なぞ。まあ登録あっての削除なので、これは気にしなくてよいかと。
	*/
	...
	*/
	...
	/*
	メモ化再帰など、サンプル入力によってはローカル環境でスタックを使い切って落ちることがあるみたい。
	AtCoder上では スタックサイズ=メモリ制限 とかになってるのか、通ったりする。
	実装に自身があればそのまま提出しても良いかも。
	参考：ABC099-C
	*/
	...
	*/
	...
	※もしくは、dp値を「この状態から始めたときの最大スコア」として元の先手/後手を意識しない実装もある。
	  実装テンプレートはTDPC-B.cppにあるdrken氏の記事URLを参照。
	  実装は簡潔になるがバグりやすいかも。
	参考問題：
	  EDPC-K
	  TDPC-B
	  ABC025-C (DP配列は自他で区別はしていない)
	...
	ABC369-F (セグ木を用いた実装が多そうだが、LISの復元も公式解説にある)
	典型90-56
	...
	*/
	...
	ABC278-E 別解 二次元いもす法
	ABC338-D
	ABC388-D シミュレートのフェーズを都度行っていく 勉強になった
	ABC419-D
	*/
	...
	*/
	...
	座標の反転、回転操作
	1-indexedにて以下が成り立つ。
	  [反転] iとN+1-iは中央を境に対象の位置となる。
	  [回転] (y,x)を90度時計回りすると(x,N+1-y)となる。
	0-indexedの場合、N+1-iではなくN-1-iとなる。
	この形の式を見たら反転、回転を考えること。
	[関連する問題]
	ABC375-C
	*/
	...
	*/
	...
	/*
	平面走査
	2つの要素があるときに、片方の要素(例えばx座標)の昇順にアクセスしていき、
	もう片方(y座標など)はセグメントツリーなどのデータ構造を用いて処理していく、という考え方。
	[参考記事]
	https://betrue12.hateblo.jp/entry/2020/09/07/224528
	...
	*/
	...
	for(i = 0; i < (1<<na); i++) {
		...
		// --iについての処理が終わったので、何らか処理--
	}
	nb = N-na;
	for(i = 0; i < (1<<nb); i++) {
		for(j = 0; j < nb; j++) {
			if( ((i>>j)&1) == 0) continue;
			// --a[j+na]を選ぶときの処理 添え字に注意--
		}
		// --iについての処理が終わったので、何らか処理--
	}
	*/
	...
	[関連する問題]
	ABC186-D
	ABC351-E マンハッタン距離は45度回転
	ABC351-F
	ABC353-C
	ABC353-D
	ABC353-E Trie木 (or ローリングハッシュ)
	ABC356-E 累積和や調和級数など、難しかったが良問
	ABC365-E 累積xor 自分がコンテスト時に考えた「差分をO(1)で計算する」方がしっくりはきた
	ABC371-E 主客転倒や余事象が絡む良問
	ABC378-E 難しかった
	ABC390-F 寄与に着目 ドチャクソ難しい
	ABC399-F 難しかった 区間の和は累積和で考える
	ABC401-F
	*/
	...
	除算について。
	mi /= 2;
	でも良いが、除算は時間がかかるので、何度も行うなら以下のように事前計算した方が早い。
	inv2 = mint(1)/2;
	mi *= inv2;
	(ABC275-Eにて、1005ms VS 16ms という差が出た)
...
void _string_(void)
{
	...
	// str.substr(10, 3);  // 第1引数が範囲外だと実行時エラー
...
// 
// priority_queueの上位互換に思えるが、setの方が定数倍が遅い。
// main()の"AtCoderにてTLEが出た場合のチェックポイント"を参照。
// 
...
void _map_(void)
{
	 ...
	 * 
	 * (2)
	 * 「同じ値を持つ要素数」をmapを用いてなんやかんやしたい場合でTLEするとき、
	 * mapではなくvector&ソートで代用するテクニックがある。ソートではなく自作ライブラリのrle()でもよい。
	 * (ex. ABC418-E)
	 */
...
void _math_(void)
{
	...
	/*
	浮動小数点数の誤差について：
	2つの集合(値は浮動小数点型)の一致判定(EPS込み)をしたい場合、値をsetやvector(&sort)でそのまま登録すると
	正しく一致判定できないことがある。
	誤差により順序が不安定になることがあるらしい。
	登録時に llround(x*1e6) として定数倍＆整数丸めすることで対処できる。
	例：ABC207-D
	 */
