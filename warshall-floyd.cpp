 ...
 * ABC375-F ワーシャルフロイドの部分更新、クエリ先読みと絡めた良問
 * ABC416-E ワーシャルフロイドの部分更新、超頂点と絡めた良問
 * 典型90-87 良問だった
 */

// ★ワーシャルフロイドの部分更新を行う場合：
//   引数にkを追加してローカル変数kの定義は削除。
//   for(k)の行も削除。
//   以上2点により「頂点kに着目して更新」ができる。辺u,vを繋ぐ辺の距離が更新された場合、u,vそれぞれで呼び出す。
//   計算量は1回あたりO(N^2).
//   入力時は[0,N)の各kについて呼び出すことで、全体でいつものO(N^3)処理となる。
//   参考：ABC416-E.cpp
