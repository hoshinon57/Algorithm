 ...
 * セグメント木で解く。
 * 
 * 左から順ではなく、hの小さい順に計算していくと、単調増加という条件を満たすのが簡易になる。
 * よってidx[]をh[]の昇順にアクセスするための添字として用意する。
 * 
 * idx[i]をi=0から順にアクセスしていき、
 *   max(dp[k] : 0<=k<idx[i]) + a[idx[i]]
 * がdp[i]の値となる。
 * max()部分はセグ木で実現する。
 */
...
int main(void)
{
	...
	vector<int> idx(N);  // h[x]の昇順にアクセスする
	iota(idx.begin(), idx.end(), 0);
	sort(idx.begin(), idx.end(), [&](int x, int y){ return h[x] < h[y];});

	for(i = 0; i < N; i++)  // idx[i]
	{
		int id = idx[i];
		ll d = seg.Query(0, id) + a[id];  // [0,id)
		seg.Update(id, d);
	}
