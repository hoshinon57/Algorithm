 ...
 * 最初に考えた方法。セグ木で解く版。
 * 
 * dp[]の定義、およびDP遷移はEDPC-M.cppと同じ。
 * ただ累積和ではなく、Range Sum Queryのセグ木で解いた。
 * …いやーだいぶ実行時間ギリギリだった。
 * 
 * いちおう供養として残しておく。
 */
...
int main(void)
{
	...
	using T = ll;
	auto fx = [](T x1, T x2) -> T { return x1+x2; };  // Range Sum Query(RSQ)
	T ex = 0;
	SegmentTree<T> seg(K+5, fx, ex);
	seg.Update(0, 1);  // DP初期値 アメ0個は1通り
	...
	for(i = 1; i <= N; i++)  // 人i
	{
		...
		{
			int l = max(j-a[i], (ll)0);
			int r = j;  // [l,r]
			dp[j] = seg.Query(l, r+1);  // [l,r+1)
			dp[j] %= MOD;
		}
		// セグ木に反映
		for(j = 0; j <= K; j++) seg.Set(j, dp[j]);
		seg.Build();
	}
	cout << seg.Get(K) << endl;
