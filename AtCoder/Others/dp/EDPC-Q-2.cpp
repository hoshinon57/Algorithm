 ...
 *   https://blog.hamayanhamayan.com/entry/2019/01/11/004701
 * 
 * dp[i][j]を以下のように定義する。
 *   i番目の花まで見て、  (i=1～N)
 *   最後の花の高さがjであるときの、  (j=1～N)
 *   美しさの総和の最大値
 * 
 * dp[i][j]への遷移は以下2パターンとなる。
 *   j =h[i]のとき : max(dp[i-1][k] | 0<=k<j) + a[i]
 *   j!=h[i]のとき : dp[i-1][j]  
 * すると、各iにてdp[i][*]が変化するのは1箇所のみとなるので、
 * dp[j]のみでデータを持つことでDP配列を使いまわせる。
 * 
 * よって以下となる。
 * dp[j] : 先頭から順に花を見ていって、各花にて最後の花の高さがjのときの、答
 * dp[j] = max(dp[k] | 0<=k<j) + A
 * 
 * max部分はセグ木で処理可能。
 * 注意として、高さは[1,N]であるので、解を出力する際に [1,N+1) での指定が必要。セグ木のサイズにも注意。
 */
...
int main(void)
{
	...
	for(i = 0; i < N; i++)  // h[i]
	{
		ll d = seg.Query(0, h[i]) + a[i];  // [0,h[i])
		seg.Update(h[i], d);
	}
	cout << seg.Query(0, N+1) << endl;  // hは最大Nまで
