 ...
 * 
 * 入力のN個のマスそれぞれを起点にして、dfs()にて隣接マスをたどっていく。
 * たどったマスはseen[]をtrueに更新していく。
 * 
 * 起点からdfs()を呼び出した回数が、求める連結個数となる。
 */
...
void dfs(int no)
{
	...
	for(int i = 0; i < 6; i++)
	{
		// 隣のマスが黒なら、そのマスへ深さ優先探索
		if(black[x+dx[i]][y+dy[i]] == 1)
		{
			// 隣のマスが何番目のマスかを、grid_idxを使って取得
			int next_no = grid_idx[x+dx[i]][y+dy[i]];

			if(seen[next_no]) continue;  // 探索済みならスルー
			dfs(next_no);
		}
	}
}
...
int main(void)
{
	...
	for(i = 0; i < N; i++)
	{
		...
		dfs(i);
		answer++;  // 各頂点を起点としてDFSを実施できた回数が、求める連結個数となる
	}
	cout << answer << endl;
