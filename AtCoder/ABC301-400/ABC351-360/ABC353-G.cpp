...
/*
 * 自力で解けず、解説を見た。セグメント木で解く。
 *   https://twitter.com/kyopro_friends/status/1789293137573343656
 *   https://atcoder.jp/contests/abc353/editorial/9953
 *   https://www.youtube.com/watch?v=u6nTE12iIGY
 * 
 * dp[i]を「最後にいる市場がiのときのX」と定義すると、
 * 市場iへの移動は、全市場j(1<=j<=N)からの移動を考えて以下となる。
 *   dp[i] = max(dp[j] - C|i-j| + Pi)
 * 場合分けで絶対値を外すと
 *   max(dp[j]+C*j - C*i + Pi) = max(dp[j]+C*j) - C*i + Pi  (i>j)
 *   max(dp[j]-C*j + C*i + Pi) = max(dp[j]-C*j) + C*i + Pi  (i<=j)
 * となる。
 * dp[j]±C*jの部分が町ごとの値であり、これを2本のセグメント木で管理すれば、
 * dp[i]はクエリごとO(logN)で求まる。
 * 
 * クエリごと求めたdp[i]について、セグ木には±C*jしたものをUpdateする必要がある。
 * 
 * 町1から順にポテンシャルがCずつ減っていくケースと、Cずつ増えていくケースがあるイメージ。
 * 
 * [どう思考すれば解法にたどり着けるか]
 * ・DPをセグメント木で高速化、っぽいところまでは思いついた。
 *   そこから先、セグ木を2本持たせる部分が思いつかなかった。
 * ・＞絶対値が入ってるのは場合分けで絶対値を外すのが鉄則
 *   であり、まずは立式を頑張るべきだったか。
 */
...
int main(void)
{
	...
	ll N, C, M; cin >> N >> C >> M;
	SegmentTree<T> seg1(N+5, fx, ex), seg2(N+5, fx, ex);
	seg1.Update(0, 0);
	seg2.Update(0, 0);
	...
	for(i = 0; i < M; i++)
	{
		ll t, p; cin >> t >> p;
		t--;
		ll tmp1 = seg1.Query(0, t) - C*t + p;  // t> jであるj [0,t)
		ll tmp2 = seg2.Query(t, N) + C*t + p;  // t<=jであるj [t,N)
		ll d = max(tmp1, tmp2);  // いわゆるdp[i], 市場iが最後にいるときのX
		chmax(ans, d);
		seg1.Update(t, d + C*t);
		seg2.Update(t, d - C*t);
	}
