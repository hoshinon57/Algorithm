 ...
 * 遅延セグメント木で解く。
 * 
 * 各操作について、箱Bのボールの数をballとすると、
 *   (ball/N)個は全ての箱に配り、
 *   (ball%N)個はB+1,B+2,... の箱に配っていく。
 * これは区間加算の遅延セグ木で処理できる。
 * 「箱Bのボールを全て取り出す」は、箱Bに -ball 個加算すればよい。
 * 
 * [ACまでの思考の流れ]
 * ・操作を考えると、
 *   ある箱の数を0にする
 *   ある区間の箱を一律加算する
 *   という処理が必要。これは遅延セグメント木で解ける。
 */
...
int main(void)
{
	...
	{
		ll b; cin >> b;
		ll ball = seg.Query(b, b+1);
		seg.Update(b, b+1, -ball);  // 箱bのボールを0個に
		// まるごとx周するものと、その余りで分ける
		seg.Update(0, N, ball/N);  // ball/N周は全体に配る
		// 余りを配る
		ll l = (b+1)%N;
		ll r = (b+ball%N+1)%N;  // [l,r)
		if(r < l)  // (N-1)->0 へ戻った
		{
			seg.Update(l, N, 1);
			seg.Update(0, r, 1);
