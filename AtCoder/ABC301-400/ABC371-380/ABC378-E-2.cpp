...
/*
 * ABC378-E.cppではrを動かしていったが、lを動かす方針で実装してみた。
 * やってることはちょうど反対側になる感じ。こちらの方が少し複雑になるかな。
 * 
 * 以下、0-indexedで記載する。
 * lを固定してM*Mのマス目で考えてみると、例としてl=1,N=4のとき
 *         A1 = S2-S1
 *      A1+A2 = S3-S1
 *   A1+A2+A3 = S4-S1
 * となる。右辺は
 *   Sx-Sl = Sx-Sl (Sx>=Slのとき) or Sx-Sl+M (Sx<Slのとき)
 * で置き換えられる。よってlを固定したときに
 *   Σ(i=(l+1)～N)Sl - Sl*(N-l) + (Sx<Slの個数)*M
 * となる。これをl=0～(N-1)について総和を取る。
 *
 * Σ(i=(l+1)～N)Sl の部分は、最初にSの累積和を計算しておき、S(l+1)を引いていくことで求められる。
 * 
 * (Sx<Slの個数)は、セグメント木にて「要素i:累積和%M=i となる要素数」として求められる。
 * 最初に各Sに対してセグ木を設定しておく必要がある。
 */
...
int main(void)
{
	...
	for(i = 1; i <= N; i++) seg_add(seg, asum[i], 1);

	ll sumsum = 0;
	for(auto &e : asum) sumsum += e;
