...
// 競プロ典型90問:29 https://atcoder.jp/contests/typical90/tasks/typical90_ac

/*
 * 遅延セグメント木で解く。
 * RMQ(max) and RUQの単位元や二項演算を用意した遅延セグメント木を用いれば、後は解くだけ。
 */
...
int main(void)
{
	int i;
	int W, N; cin >> W >> N;
	// RMQ and RUQ(Range Maximum Query and Range Update Query)
	// ------
	using X = int;
	using M = int;
	...
	X ex = 0;
	M em = numeric_limits<M>::min();
	// ------
	LazySegmentTree<X, M> seg(W+5, fx, fa, fm, nullptr, ex, em);
	for(i = 0; i < N; i++)
	{
		int l, r; cin >> l >> r;
		r++;  // 半開区間 [l,r) への変換
