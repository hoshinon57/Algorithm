...
/*
 * inlineDPとセグ木で解いた。公式解説が近かった。
 * 
 * 複数の要素があるときは、ソートすると見通しが良くなることがある。[典型]
 * wについてソートしておくことで、i<jならwi<=wjとなるので
 * 先頭から見ていけば "wi=wjのケースを除いて(後述)" wについては考慮不要になる。
 * 
 * i個目までみて、外側の箱の高さh=jであるようなときの、箱の入れ子の個数
 * をDPとして考えてみる。
 *   dp[i][hi] = max(dp[i-1][0]～dp[i-1][hi-1]) + 1
 * として更新できる。
 * ここで、iについてhiのみ変化することと、
 * max()の部分を区間最大のセグ木で高速化することを考える。
 * 
 * 前述の "wi=wjのケースを除いて(後述)" がやっかい。
 * 自分は公式解説にもある以下の方針で実装した。
 * ＞その他に、hが同じ箱を計算している間はBITをupdateせず、
 * ＞hが変わるとそこでupdateを全て行うという方法もある
 * もしくは、同じwならhの降順にソートしておくと、先頭から順に上手く処理できる。なるほど。
**/
...
int main(void)
{
	...
	const int MXH = 1e5;
	SegmentTree<T> seg(MXH+5, fx, ex);
	for(i = 0; i < N; )
	{
		// [st,ed) は同じw
		int st = i, ed;
		vector<pair<int,int>> li;  // {h, cnt}
		for(ed = st; ed < N; ed++)
		{
			if(b[st].first != b[ed].first) break;
			auto [w,h] = b[ed];
			auto cnt = seg.Query(0, h) + 1;
			li.push_back({h, cnt});  // hに対してcntは同一なので、ダブっても良い
		}
