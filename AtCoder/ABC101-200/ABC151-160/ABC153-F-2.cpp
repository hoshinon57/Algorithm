...
int main(void)
{
	...
	M em = 0;
	////
	LazySegmentTree<X, M> seg(N, fx, fa, fm, fp, ex, em);  // 要素iのモンスターのHP 座標は別配列x[]にて管理
	...
	for(i = 0; i < N; i++)  // モンスター:i
	{
		...
		ll count = (seg.Query(i,i+1)+(A-1)) / A;  // モンスターiを撃破するために使う爆弾の数 HP/Aを切り上げ
		seg.Update(i, idx+1, -(A*count));
		answer += count;
	}
	cout << answer << endl;
