...
/*
 * 公式解説における「ハッシュによる解法」で解いたもの。
 *   https://atcoder.jp/contests/abc424/editorial/13900
 * 線分の端点に乱数を割り当てて、区間[A,B]のxorが0かどうかで判定する。
 * クエリである区間について、既存の各線分が0 or 2個含まれていればOK,のため。
 * 
 * ※コンテスト時はこれに近く、
 *     RMQ and RAQの遅延セグ木を使用
 *     乱数を1つ取得し、区間[A,B]に加算
 *     地点AとBの値が同じなら線分を引ける
 *   として解いた。
 *   https://atcoder.jp/contests/abc424/submissions/69502112
 *   符号あり整数のオーバーフローは未定義のため、加算ではなくxorの方が安全。
 *   もしくはオーバーフローしないような乱数の範囲にするとか。
**/
...
int main(void)
{
	...
	while(Q > 0)
	{
		...
		bool ans = (seg.Query(a, b+1) == 0);
		YesNo(ans);
		if(ans)
		{
			auto r = rng_64(0, INF64);
			seg.Update(a, r);
			seg.Update(b, r);
		}
	}
