...
/*
 * 証明はさておき、区間和取得＆区間更新の遅延セグ木で「やるだけ」問題。
 * クエリL,Rを半開区間の[l,r)に変換したら、[l,r)をその区間の平均に更新すればよい。
 * 
 * [l,r)を皿xに集めるのを、[l,r)にならす として考える的な。
 * 
 * 証明は期待値の線形性になるらしい。
**/
...
int main(void)
{
	...
	for(i = 0; i < N; i++)
	{
		cout << seg.Query(i, i+1).val() << " ";  // この問題、入力例の末尾に空白がある
	}
	cout << endl;
