 ...
 * 平面走査（かな？）のセグメント木で解く。
 * 
 * 各種解説では低い方から、「そこからスタートしたときに何回移動できるか？」としているが、
 * 本実装では高い方から、「そこに来るまでに最大何回移動できるか？」とした。
 * (特に理由はないが、考察時にそっち側だったため)
 * 
 * 基本は公式解説の通りで、もらうDPで [idx-R,idx+R] の最大値をセグメント木で高速に取得する。
 * このときDの条件がジャマで、これはセグメント木への反映をDだけ遅らすことで対応できる。
 * 
 * [どう思考すれば解法にたどり着けるか]
 * ・コンテスト時、もらうDPではなく配るDPのイメージで考えていた。
 *   この場合chmax()のセグメント木が欲しくなり、これはSegment Tree Beatsという特殊なデータ構造が必要になるらしい。
 *   そうではなく、もらうDPで考えるとrange maxのセグメント木で実現できる。(Dに関する検討は必要だが)
 * 
 *   DPをセグメント木で高速化するとき、
 *     配るDP：chmaxが必要になり、自分が持ってるライブラリでは解けない
 *     もらうDP：range maxのセグメント木で解ける
 *   という差異が出てくる。というのを覚えておく。
**/
...
int main(void)
{
	...
	ll i;
	ll N, D, R; cin >> N >> D >> R;
	vector<ll> hidx(N+5);  // hidx[h]:高さhのidx(横の位置)
	vector<ll> h(N); for(i = 0; i < N; i++) {cin >> h[i]; hidx[h[i]] = i;}

	vector<pair<ll,ll>> hi(N);  // {H,idx}
	for(i = 0; i < N; i++) hi[i] = {h[i], i};
	sort(hi.begin(), hi.end(), greater<pair<ll,ll>>());  // h[i]の高い方からソート

	SegmentTree<T> seg(N+5, fx, ex);
	vector<ll> ans(N);
	for(i = 0; i < N; i++)  // i番目に高い要素
	{
		auto[hh,idx] = hi[i];
		// 高さhh+Dをセグ木へ反映
		if(hh+D <= N)
		{
			seg.Update(hidx[hh+D], ans[hidx[hh+D]]);
		}
		ll l = max(idx-R, 0LL);
		ll r = min(idx+R, N);
		ans[idx] = seg.Query(l, r+1) + 1;
	}
