...
// ワーシャルフロイド法の気持ちになって解く。
// 各k,i,jについてdist[i][j]の更新後、
// i->jへの経路があれば(自分のライブラリではINF64以外なら)答に加算する。
...
// 負閉路があればtrueを返す。
bool Warshall_Floyd(vector<vector<ll>> &dist, ll &ans)
{
	int i, j, k;
	int N = dist.size();
	...
	for(k = 0; k < N; k++)
	{
		for(i = 0; i < N; i++)
		{
			for(j = 0; j < N; j++)
			{
				...
				if(dist[i][j] != INF64) ans += dist[i][j];
			}
		}
	}
