 ...
 * ・非負mod smod
 * ・[デバッグ用]R(mod M)を有理数P/Qに復元 convmod
 * ・乱数生成 rng_64
 * ・N*N配列を回転 rotate
 ...
 * ・文字列S,Tについて、Sに1回だけ挿入、削除、変更操作を行ってTに一致できるかを判定 -> ABC386-C.cppを参照
 */
...
// [デバッグ用]整数Rをmod MでP/Qとなる有理数に変換する
// P,Qは[1,lim]の範囲で探索する(負数は調べない)。候補のうち|P|+|Q|が最小のものを出力する。
// よってだいたい合ってる、負数は見つけられない、という程度のデバッグ用。
// 計算量はO(lim^2).
// 参考：https://marshmallow-qa.com/messages/f83cbb18-d818-426d-b13d-3560789f9758?hl=ja
// 例：R=499122178は3/2, R=598946612は1/5, R=3は3.
void convmod(ll R, ll M = 998244353)
{
	vector<vector<ll>> ret;  // {p+q, p, q}
	const ll lim = 5000;
	for(ll p = 1; p <= lim; p++) {
		for(ll q = 1; q <= lim; q++) {
			if((q*R)%M == p)
				ret.push_back({p+q, p, q});  // 探索範囲は正のため、p+qに絶対値は不要
		}
	}
	sort(ret.begin(), ret.end());  // p+qでソート
	if(ret.size() != 0) {
		cout << R << "=" << ret[0][1] << "/" << ret[0][2];
		cout << endl;
	}
	else
...
// [verify]ABC336-C, ABC234-C, ABC372-B, ABC414-C
